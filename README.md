...
api_instance = swagger_client.AdminResourceApi(swagger_client.ApiClient(configuration))
...
Start Conductor server by running this command at the repo root folder (`/conductor`):

```shell
$ ./gradlew bootRun
```

Start Conductor UI by running this command at the UI folder (`/conductor/ui`):
...
```shell
$ virtualenv conductor-python-client
```

Install Conductor Python Client:
...
* URL example: `http://localhost:5001/execution/8ff0bc06-4413-4c94-b27a-b3210412a914`